...
for index in tqdm(range(8, 10)):
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            },
            {
                ...
                "Now, let's firstly fcous on a special background noise types: driving or traffic. \n"
                ...
                '{text: "Where\'s a good place to eat around here?", background: "driving and traffic", reply: ["Since you\'re on the road, I\'ll find something easy to access. There\'s a highly-rated grill just off the next exit. Want me to guide you there?"], reason: "..."}\n'
                "OK, now it's your turn! Please provide examples of how a speech dialogue model would adapt its responses based on different environmental sounds, let's firstly fcous on a special background types: driving or traffic.\n"
                "Again, the scene we are focusing on is an adult conversation but the reply may change due to the presence of driving or traffic. \n"
    ...
    with open(
        "temp_jsons/gpt_env_examples_txt/driving_{}.txt".format(str(index)), "w"
    ) as f:
        f.write(response)
    ...
    # save the response to a file
    with open(
        "temp_jsons/gpt_env_examples/driving_{}.json".format(str(index)), "w"
    ) as f:
