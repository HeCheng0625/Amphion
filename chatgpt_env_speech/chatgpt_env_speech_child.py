...
for index in tqdm(range(0, 10)):
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            },
            {
                ...
                "Now, let's firstly fcous on a special background noise types: children's voice (children speaking or children playing). \n"
                ...
                '{text: "Tell me about the risks of gambling?", background: "children\'s voice", reply: ["It sounds like it might be family time right now! I can recommend some great family-friendly movies instead. How about something animated or a classic children\'s adventure?", reason: "Not suitable for discussing gambling in front of children."]}\n'
                "More examples about other background noise: \n"
                '{text: "What should I wear today?", background: "raining and thundering", reply: ["Given the rainy and windy weather outside, I recommend wearing a waterproof jacket and sturdy, water-resistant shoes. Would you like a reminder to bring an umbrella?"], reason: "Since there are sounds of rain and thunder in the background, it is recommended to bring an umbrella."}\n'
                "OK, now it's your turn! Please provide examples of how a speech dialogue model would adapt its responses based on different environmental sounds, let's firstly fcous on a special background types: children's voice.\n"
                "Again, the scene we are focusing on is an adult conversation but the reply may change due to the presence of children's voices in the background. \n"
                'And in my examples, each "reply" contains only one response, you should provide 5 proper responses for each example. \n'
                "I wish a part of questions themselves were more adult-oriented, a part of. \n"
    ...
    with open(
        "temp_jsons/gpt_env_examples_txt/child_{}.txt".format(str(index)), "w"
    ) as f:
        f.write(response)
    ...
    # save the response to a file
    with open("temp_jsons/gpt_env_examples/child_{}.json".format(str(index)), "w") as f:
